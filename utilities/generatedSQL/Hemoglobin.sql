...
FROM all_program_entire_enrolment lpe
  ...
  LEFT OUTER JOIN gender g ON i.gender_id = g.id
WHERE cast(lpe.obs ->> ''''f9ecabbc-2df2-4bfc-a6fa-aa417c50e11b'''' AS FLOAT) > 7
      AND cast(lpe.obs ->> ''''f9ecabbc-2df2-4bfc-a6fa-aa417c50e11b'''' AS FLOAT) <= 10'')
UNION ALL
SELECT
  ''10.1 - 11.9''                                          rowid,
  address_type || '' '' || gender AS                             attribute,
...
WHERE cast(lpe.obs ->> ''''f9ecabbc-2df2-4bfc-a6fa-aa417c50e11b'''' AS FLOAT) > 10
      AND cast(lpe.obs ->> ''''f9ecabbc-2df2-4bfc-a6fa-aa417c50e11b'''' AS FLOAT) < 12'')
UNION ALL
SELECT
  ''12''                                          rowid,
  address_type || '' '' || gender AS                             attribute,
...
FROM all_program_entire_enrolment lpe
  ...
  LEFT OUTER JOIN gender g ON i.gender_id = g.id
WHERE cast(lpe.obs ->> ''''f9ecabbc-2df2-4bfc-a6fa-aa417c50e11b'''' AS FLOAT) > 12
      AND cast(lpe.obs ->> ''''f9ecabbc-2df2-4bfc-a6fa-aa417c50e11b'''' AS FLOAT) <= 13'')
UNION ALL
SELECT
  ''> 13''                                          rowid,
  address_type || '' '' || gender AS                             attribute,
...
FROM all_program_entire_enrolment lpe
  ...
  LEFT OUTER JOIN address_level a ON i.address_id = a.id
  LEFT OUTER JOIN gender g ON i.gender_id = g.id
WHERE cast(lpe.obs ->> ''''f9ecabbc-2df2-4bfc-a6fa-aa417c50e11b'''' AS FLOAT) > 13'')') AS (
rowid TEXT,
"All Female" TEXT,
"All Male" TEXT,
"All Total" TEXT,
